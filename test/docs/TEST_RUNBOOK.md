...
- пустая база показывает нулевой итог и не создаёт второй источник денег;
- внесённые продажи дают приход, а закупки и траты — выплаты;
- ручного журнала в отчёте нет, поэтому одну сумму нельзя учесть дважды;
- движения с 2025 года и их суммы по категориям считаются предсказуемо;
- границы срока и одинаковое время операций дают стабильный порядок и верный итог;
- каждая строка отчёта хранит ссылку на исходную продажу, закупку или трату.

...
private fun baseModule(dependencies: IProductFormDependencies) = module {
    ...
    single<ItemListRepository<Document, DocumentType>>(named(ItemListType.Document.name)) {dependencies.documentListRepository  }
    single<ItemListRepository<Product, ProductType>>(named(ItemListType.Product.name)) {dependencies.productListRepository  }

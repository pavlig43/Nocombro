...
/**
 * Настройки времени работы для S3-совместимого объектного хранилища.
 *
 * Основной сценарий для desktop-приложения: читать конфиг из `%APPDATA%\Nocombro\s3.properties`,
 * чтобы хранить его рядом с остальными локальными рантайм-файлами приложения.
 *
 * Ещё остаётся fallback на env и JVM properties, чтобы не ломать ручные dev-сценарии.

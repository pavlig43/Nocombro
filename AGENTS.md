...
Если пользователь просит код-ревью, тесты не запускать без отдельной просьбы.

Пока запущена desktop-версия Nocombro, не собирать Android-версию: сборка не пройдёт. Сначала закрыть desktop-приложение.

Если правка мала и не затрагивает сигнатуры, зависимости, DI, схему БД или сборочные скрипты, Gradle-сборку не запускать. Для замены нескольких строк текста или подписей достаточно проверить diff и сам файл.
...
- В новом и изменяемом Kotlin-коде не использовать `init`-блоки.
- Подписки между компонентами передавать через явный обработчик владельца либо собирать производный `StateFlow` через `map`, `combine` и `stateIn`.
- Не прятать запуск корутин в инициализатор свойства: это тот же скрытый побочный эффект.

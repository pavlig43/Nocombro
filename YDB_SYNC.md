...
- Doctor S3 cleanup сравнивает S3 с активными строками mirror `file`, локальными
  файлами и реестром незавершённых загрузок. Перед удалением remote snapshot
  загружается повторно. Cleanup блокируется при ошибке sync или незавершённых
  локальных изменениях и загрузках.
